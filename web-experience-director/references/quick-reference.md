# Quick Reference — Full Rule Set (all 11 categories)
...
### 11. Motion, 2.5D, And Spatial Work
- `experience-level` - Choose routine UI, cinematic 2.5D, pre-rendered motion, or live 3D from the user need
- `visual-premise` - Name a concrete product-linked premise before choosing a visual language
- `2.5d-before-webgl` - Use layered DOM and CSS when depth is editorial rather than spatial inspection
- `semantic-canvas-boundary` - Keep essential content and controls in semantic DOM outside canvas
- `camera-contract` - Define opening framing, allowed range, presets, reset path, and stable content rail
- `scene-fallback` - Provide a useful static gallery or DOM alternative for unsupported or lost graphics
- `demand-rendering` - Render only while a scene changes and invalidate demand-rendered React Three Fiber scenes explicitly
- `gpu-budget` - Cap render scale, texture size, draw calls, and post-processing on constrained hardware
- `resource-ownership` - Dispose geometries, materials, textures, render targets, and removed models by ownership
- `pointer-cancellation` - Reset drag state on pointerup, pointercancel, and lost pointer capture
- `drag-alternative` - Provide visible click, tap, and keyboard alternatives for scene manipulation
- `spatial-reduced-motion` - Replace camera travel, parallax, scaling, and loops with a stable frame or direct state change
