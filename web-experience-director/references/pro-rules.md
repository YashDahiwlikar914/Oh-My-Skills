...
Load this file before final delivery of native, mobile, web, desktop, or spatial UI, or when the user reports the UI "doesn't look professional" and the cause isn't obvious from the priority table in SKILL.md.

The rules below cover shared visual quality, native and mobile details, web interaction, and spatial enhancement. Use `references/quick-reference.md` for the full web and app rule set.
...
### Web And Spatial Enhancement
- [ ] Essential text, navigation, state, price, forms, and actions remain in semantic DOM outside canvas
- [ ] Informative visual content has useful alternative text, captions, or adjacent explanation
- [ ] Custom scene controls expose an accessible name, role, state, and value where relevant
- [ ] Every author-controlled drag action has a click or tap and keyboard alternative
- [ ] A poster, static image gallery, or DOM path remains usable when WebGL is unavailable or context is lost
- [ ] Canvas size follows CSS layout and internal pixel count is capped for constrained devices
- [ ] The scene does not render continuously while idle unless the product need requires it
- [ ] Owned geometries, materials, textures, render targets, and models are disposed on removal
- [ ] Normal page scrolling remains available around embedded scenes
- [ ] `prefers-reduced-motion` removes nonessential camera travel, parallax, scaling, and looping motion
- [ ] A constrained mobile profile has been tested for frame pacing, input response, memory growth, and heat
