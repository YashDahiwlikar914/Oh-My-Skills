...
from experience_rules import compose_experience
...
SEARCH_CONFIG = {
    ...
    "website-types": {"max_results": 1},
    "design-languages": {"max_results": 1},
    "layout-structures": {"max_results": 1},
}
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        experience = compose_experience(query)
        ...
        return {
            ...
            "reasoning_default": reasoning.get("is_default", False),
             "source_identities": {
                 ...
                 "website_type": experience["ingredients"]["website_type"].get("id") or None,
                 "design_language": experience["ingredients"]["design_language"].get("id") or None,
                 "layout_structure": experience["ingredients"]["layout_structure"].get("id") or None,
             },
            "source_derivations": {
...
def format_ascii_box(design_system: dict) -> str:
    ...
    experience = design_system.get("experience", {})
    ...
    if experience:
        lines.append(section_header("EXPERIENCE", BOX_WIDTH + 1))
        lines.append(f"│  Level: {experience.get('experience_level', '')}".ljust(BOX_WIDTH) + "│")
        lines.append(f"│  Status: {experience.get('status', '')}".ljust(BOX_WIDTH) + "│")
        plan = experience.get("generated_plan", {})
        if plan.get("reason"):
            for line in wrap_text(f"Reason: {plan['reason']}", "│     ", BOX_WIDTH):
        ...
        for key, label in labels.items():
            ingredient = experience.get("ingredients", {}).get(key, {})
            name = ingredient.get("name") or ingredient.get("requested") or "unmatched"
            lines.append(f"│  {label}: {name} [{ingredient.get('status', '')}]".ljust(BOX_WIDTH) + "│")
...
def format_markdown(design_system: dict) -> str:
    ...
    if experience:
        ...
        lines.append(f"- **Level:** {experience.get('experience_level', '')}")
        lines.append(f"- **Status:** {experience.get('status', '')}")
        plan = experience.get("generated_plan", {})
        if plan.get("reason"):
        ...
        for key, label in labels.items():
            ingredient = experience.get("ingredients", {}).get(key, {})
            name = ingredient.get("name") or ingredient.get("requested") or "unmatched"
            lines.append(f"- **{label}:** {name} ({ingredient.get('status', '')})")
        lines.append("")
...
def format_master_md(design_system: dict) -> str:
    ...
    if experience:
        lines.append(f"**Experience Level:** {experience.get('experience_level', '')}")
        lines.append(f"**Experience Status:** {experience.get('status', '')}")
        for key, label in (
            ("website_type", "Website Type"),
            ("design_language", "Design Language"),
            ("layout_structure", "Layout Structure"),
