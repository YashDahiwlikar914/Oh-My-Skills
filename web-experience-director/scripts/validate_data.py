...
# -*- coding: utf-8 -*-
"""
Data integrity guardrail for Web Experience Director. Stdlib-only, no pytest dependency,
...
CORE_PROVENANCE_FILES = {
    ...
    "website-types.csv", "design-languages.csv", "layout-structures.csv",
    "experience-rules.json",
}
TAXONOMY_DOMAINS = {
    "website-types": {
        "required": {"ID", "Name", "Family", "Era", "Intent", "Keywords",
                      "Primary Pages", "Primary Structures", "Responsive Notes",
                      "Accessibility Notes", "Anti-Patterns", "Sources", "Status",
                      "Verified At"},
        "source_file": "website-types.csv",
    },
    "design-languages": {
        "required": {"ID", "Name", "Family", "Era", "Intent", "Keywords",
                      "Typography", "Color And Surface", "Motion And Depth", "Best Fit",
                      "Accessibility Notes", "Anti-Patterns", "Sources", "Status",
                      "Verified At"},
        "source_file": "design-languages.csv",
    },
    "layout-structures": {
        "required": {"ID", "Name", "Family", "Intent", "Keywords",
                      "Primary Content Fit", "Responsive Notes", "Accessibility Notes",
                      "Implementation Notes", "Anti-Patterns", "Sources", "Status",
                      "Verified At"},
        "source_file": "layout-structures.csv",
    },
}
TAXONOMY_STATUSES = {"active", "candidate", "deprecated"}
EXPERIENCE_LEVELS = {"routine-ui", "2.5d", "interactive-3d", "pre-rendered-motion"}
TAXONOMY_SOURCE_FILES = {
    contract["source_file"] for contract in TAXONOMY_DOMAINS.values()
}
...
OFFICIAL_SOURCE_HOSTS = {
    ...
    "developer.mozilla.org", "developers.google.com", "design.google",
    "designsystem.digital.gov", "design-system.service.gov.uk",
    "fluent2.microsoft.design", "github.com", "greensock.com", "gsap.com",
    "m3.material.io", "metmuseum.org", "www.metmuseum.org", "nngroup.com", "www.nngroup.com",
    "opensource.adobe.com", "react.dev", "s2.spectrum.adobe.com", "shopify.dev",
    "tailwindcss.com", "threejs.org", "www.w3.org", "vam.ac.uk", "tate.org.uk",
    "getty.edu", "britannica.com", "si.edu", "loc.gov", "moma.org",
    "americanart.si.edu", "webkit.org", "baymard.com", "webaim.org",
    "uxpatterns.dev", "www.webdesignmuseum.org",
}
...
def _check_catalog_summary(summary, licenses, phosphor, problems):
    ...
    expected = {
        ...
        "websiteTypes": len(_read_rows(DATA_DIR / "website-types.csv")[1]),
        "designLanguages": len(_read_rows(DATA_DIR / "design-languages.csv")[1]),
        "layoutStructures": len(_read_rows(DATA_DIR / "layout-structures.csv")[1]),
        "experienceRules": len(json.loads((DATA_DIR / "experience-rules.json").read_text(encoding="utf-8")).get("rules", [])),
    ...
    taxonomy_summary = summary.get("taxonomy") if isinstance(summary.get("taxonomy"), dict) else {}
    taxonomy_keys = {
        "website-types": "websiteTypes",
        "design-languages": "designLanguages",
        "layout-structures": "layoutStructures",
    }
    for domain, summary_key in taxonomy_keys.items():
        rows = _read_rows(DATA_DIR / TAXONOMY_DOMAINS[domain]["source_file"])[1]
        expected_taxonomy = {
            "total": len(rows),
            "active": sum(row.get("Status") == "active" for row in rows),
            "candidate": sum(row.get("Status") == "candidate" for row in rows),
        }
        if taxonomy_summary.get(summary_key) != expected_taxonomy:
            problems.append(f"[catalog:summary] stale taxonomy count for {summary_key}")
...
def _check_taxonomy_contract(domain_rows, problems):
    for domain, contract in TAXONOMY_DOMAINS.items():
        rows = domain_rows.get(domain, [])
        if not rows:
            problems.append(f"[taxonomy:{domain}] no rows found")
            continue
        identities = set()
        for row in rows:
            identity = row.get("ID", "")
            if not STYLE_ID.fullmatch(identity):
                problems.append(f"[taxonomy:{domain}] invalid ID '{identity}'")
            if identity in identities:
                problems.append(f"[taxonomy:{domain}] duplicate ID '{identity}'")
            identities.add(identity)
            for field in contract["required"]:
                if not row.get(field, "").strip():
                    problems.append(f"[taxonomy:{domain}:{identity}] missing {field}")
            status = row.get("Status", "")
            if status not in TAXONOMY_STATUSES:
                problems.append(f"[taxonomy:{domain}:{identity}] invalid Status '{status}'")
            if not _valid_date(row.get("Verified At", "")):
                problems.append(f"[taxonomy:{domain}:{identity}] invalid Verified At")
            sources = [source.strip() for source in row.get("Sources", "").split("|") if source.strip()]
            for source in sources:
                parsed = urlsplit(source)
                if parsed.scheme != "https" or not parsed.netloc:
                    problems.append(f"[taxonomy:{domain}:{identity}] invalid source '{source}'")


def _check_experience_rules_contract(payload, problems):
    if not isinstance(payload, dict) or payload.get("schemaVersion") != 1:
        problems.append("[experience-rules] schemaVersion 1 object is required")
        return
    rules = payload.get("rules")
    if not isinstance(rules, list) or not rules:
        problems.append("[experience-rules] rules must be a non-empty array")
        return
    seen = set()
    for rule in rules:
        if not isinstance(rule, dict):
            problems.append("[experience-rules] every rule must be an object")
            continue
        rule_id = rule.get("id", "")
        if not STYLE_ID.fullmatch(rule_id) or rule_id in seen:
            problems.append(f"[experience-rules] invalid or duplicate id '{rule_id}'")
        seen.add(rule_id)
        if rule.get("experienceLevel") not in EXPERIENCE_LEVELS:
            problems.append(f"[experience-rules:{rule_id}] invalid experienceLevel")
        for field in ("signals", "constraints"):
            values = rule.get(field)
            if not isinstance(values, list) or not values or not all(
                    isinstance(value, str) and value.strip() for value in values):
                problems.append(f"[experience-rules:{rule_id}] {field} must be non-empty strings")
        if not isinstance(rule.get("fallback"), str) or not rule["fallback"].strip():
            problems.append(f"[experience-rules:{rule_id}] fallback is required")
    default = payload.get("default")
    if not isinstance(default, dict) or default.get("experienceLevel") not in EXPERIENCE_LEVELS:
        problems.append("[experience-rules] default experienceLevel is invalid")
...
def _check_core_data_contract(domain_rows, problems):
    ...
    _check_taxonomy_contract(domain_rows, problems)
...
def _valid_dataset_source_key(source_file, source_key, identity, problems):
    ...
    if source_file == "experience-rules.json":
        if not isinstance(scope, str) or not scope.startswith("Rules "):
            problems.append(f"[provenance] experience rule scope is invalid for {identity}")
            return False
        try:
            payload = json.loads((DATA_DIR / source_file).read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError, UnicodeDecodeError):
            problems.append(f"[provenance] unreadable experience rules for {identity}")
            return False
        if scope.split(";", 1)[0][6:].strip() != "all" or not payload.get("rules"):
            problems.append(f"[provenance] experience rule scope does not bind all rules for {identity}")
            return False
        if not all(field in scope for field in ("signals", "constraints", "fallback")):
            problems.append(f"[provenance] experience rule scope must bind rule fields for {identity}")
            return False
        return True
    ...
    if not isinstance(scope, str):
        ...
        return False
    if source_file in TAXONOMY_SOURCE_FILES:
        if not scope.startswith("ID "):
            problems.append(f"[provenance] taxonomy scope must bind IDs for {identity}")
            return False
        row_ids = {row.get("ID", "") for row in rows if row.get("ID", "")}
        requested = scope.split(";", 1)[0][3:].strip()
        if requested != "all" and not set(value.strip() for value in requested.split(",") if value.strip()) <= row_ids:
            problems.append(f"[provenance] taxonomy scope references unknown IDs for {identity}")
            return False
        if not any(re.search(rf"\b{re.escape(header)}\b", scope, re.I)
                   for header in headers if header != "ID"):
            problems.append(f"[provenance] taxonomy scope must bind source fields for {identity}")
            return False
        return True
    if not scope.startswith("No "):
        problems.append(f"[provenance] dataset scope must bind rows for {identity}")
        return False
...
def validate():
    ...
    rules_path = DATA_DIR / "experience-rules.json"
    try:
        rules = json.loads(rules_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        problems.append(f"[experience-rules] invalid JSON: {error}")
    else:
        _check_experience_rules_contract(rules, problems)
    for row in landing:
