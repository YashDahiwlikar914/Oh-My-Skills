---
name: omarchy
description: Use when customizing an installed Omarchy Linux desktop or its user configuration, including Hyprland, Waybar, Walker, Alacritty, Foot, Kitty, Ghostty, Mako, terminals, themes, wallpaper, monitors, keybindings, window rules, idle, lock screen, screenshots, reminders, displays, user-facing omarchy commands, or files under ~/.config/hypr/, ~/.config/waybar/, ~/.config/walker/, ~/.config/alacritty/, ~/.config/foot/, ~/.config/kitty/, ~/.config/ghostty/, ~/.config/mako/, or ~/.config/omarchy/. Do not use for Omarchy source development or `omarchy dev` workflows.
---
