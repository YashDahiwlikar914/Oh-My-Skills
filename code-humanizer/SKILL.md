---
name: code-humanizer
description: Use when reviewing or revising source code, tests, scripts, configuration, or code-focused documentation that is over-engineered, inconsistent with the repository, difficult to trust or maintain, or AI-generated or heavily AI-edited. Do not use for prose-only editing.
license: MIT
metadata:
  version: "1.0.0"
---
...
Humanize implementation code, tests, configuration, scripts, and code-focused
documentation. Do not rewrite behavior merely to change its visual style. Do
not add quirks, typos, fake history, arbitrary naming, or inconsistent
formatting. Do not claim that code is human because a detector accepts it.
...
unsafe boundary, a misleading name, a comment that lies, or a test that would
...
Delete comments that narrate syntax or repeat the function name. Keep comments
that explain why a surprising choice exists, what invariant must remain true,
what external behavior cannot be expressed in code, or what security and
concurrency constraint shaped the implementation.

A good comment is specific to this code. Replace vague comments such as
"Handle edge cases" with the actual invariant or remove them. Never write a
comment that describes a change as if the reader has to know the previous
version, unless the file is a changelog or migration document.
...
Reduce ceremony in both code and the response. Do not add a new layer to make
the code look considered. Do not add a paragraph explaining an obvious three
line change. A small diff with a clear reason is more credible than a polished
tour of every edit.
...
1. State the concrete problems found.
2. State the important changes made.
