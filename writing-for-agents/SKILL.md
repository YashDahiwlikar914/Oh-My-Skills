...
description: Use when designing or revising information architecture in agent-facing documents, especially context pointers, progressive disclosure, reference splitting, completion criteria, leading terms, or pruning.
---
...
When the target is a skill, use `writing-skills` for `SKILL.md` authoring and `evaluating-skills` for trigger or performance evaluation. Read [`SKILL-MECHANICS.md`](SKILL-MECHANICS.md) when the information architecture decision involves frontmatter, invocation choice, or router skills.

## Scope Boundary

Use this skill for information architecture in agent-facing documents.

- Shape context pointers and their trigger branches.
- Decide what belongs in the main file and what belongs behind a reference pointer.
- Split mutually exclusive branches by disclosure or invocation.
- Define completion criteria that make each step checkable.
- Prune duplication, stale material, and no-op instructions.

Use `writing-skills` for skill authoring and pressure testing. Use `evaluating-skills` for trigger evaluation, benchmarks, and version comparisons.
