...
Install globally with the cross-agent skills CLI so Human Writing is available in every project:

```bash
npx skills add blader/humanizer --global
```

Update an existing Human Writing install:
...
Claude Code users can also install the original package as a plugin:
...
The skill is then invoked as `/humanizer:humanizer` because the plugin package keeps its original upstream name.
...
Rewrites also follow a plain-punctuation standard. The final text contains no em dashes, no colons or parentheses in prose, no culture-bound idioms, and deliberate variation in sentence length.
...
| # | Pattern | Before | After |
|---|---------|--------|-------|
| 7 | **AI vocabulary** | "Actually... additionally... testament... landscape... showcasing" | Swap for the plain version, "use", "build", "show", "solid" |
...
| # | Pattern | Before | After |
|---|---------|--------|-------|
| 14 | **Em/en dashes** | "institutions—not the people—yet this continues—" | Cut them, using periods, commas, or conjunctions |
...
| # | Pattern | Before | After |
|---|---------|--------|-------|
| 34 | **Culture-bound idioms** | "kill two birds with one stone and hit the ground running" | "handles two jobs at once, so teams can start quickly" |
| 35 | **Colons and parentheses** | "one job (making configs painless): no setup" | Split into plain sentences |
| 36 | **Memory-dependent openers** | "This obsession is precisely why the rewrite succeeded" | "succeeded because the team cared about speed" |
| 37 | **Thesis-first paragraphs** | "Collaboration is one of the most important values..." | Start with the specific thing |
| 38 | **Even, mid-length cadence** | Every sentence the same mid length | Vary the rhythm on purpose |
...
- **3.0.0** - Merged a plain-language writing standard into the skill. Added patterns #34-38 covering culture-bound idioms, colons and parentheses in prose, memory-dependent openers, thesis-first paragraphs, and uniform sentence cadence. Expanded #7 with a full replacement table and a jargon-stacking note, removed colons and parentheses as em dash replacements in #14, and added a false-modesty note to #24. 38 patterns total.
